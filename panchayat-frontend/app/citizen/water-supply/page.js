          ...
          <div className="space-y-1">
            ...
            {activeAlerts.map((alert, idx) => (
              <p key={idx} className="text-xs font-semibold leading-relaxed text-rose-600/90">
                {alert.area}: {alert.notes || "Emergency pipeline repairs are in progress. Timing is affected."}
              </p>
            ))}
          </div>
        </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {loading ? (
                ...
                schedules.map((sched) => (
                  <div key={sched.id} className="p-6 bg-slate-50 hover:bg-slate-100/50 transition-colors border border-slate-100 rounded-3xl flex items-center justify-between gap-6">
                    <div className="flex items-center gap-4">
                      <div className={`p-3 rounded-2xl ${sched.status === 'active' ? 'bg-blue-100 text-blue-600' : 'bg-rose-100 text-rose-600'}`}>
                ...
                tanks.map((tank) => (
                  <div key={tank.id} className="space-y-2 border-b border-slate-50 pb-4 last:border-0 last:pb-0">
                    <div className="flex justify-between items-center text-xs font-black uppercase tracking-widest">
                      ...
                      <span className="text-blue-600 font-bold">{tank.capacity.toLocaleString()} Liters</span>
                    </div>
