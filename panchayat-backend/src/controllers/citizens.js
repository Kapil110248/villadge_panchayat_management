...
exports.getCitizens = async (req, res) => {
  ...
  try {
    const citizens = await prisma.user.findMany({ where: { role: "citizen" }, include: { citizenprofile: true }, orderBy: { created_at: 'desc' } });
...
exports.createCitizen = async (req, res) => {
  ...
  try {
    ...
    const newUser = await prisma.user.create({ data: { email: data.email, password_hash: data.password, role: "citizen", full_name: data.full_name, mobile: data.mobile, is_active: data.status === "Active", updated_at: new Date(), citizenprofile: { create: { gender: data.gender.toLowerCase(), village: data.ward, aadhaar_number: data.aadhaar_number || `PENDING-${uuidv4().substring(0, 8)}`, avatar_url: data.avatar_url, address: data.address, date_of_birth: data.dob ? new Date(data.dob) : null, father_name: data.father_name } } } });
...
exports.updateCitizen = async (req, res) => {
  ...
  try {
    ...
    const data = req.body;
    const user = await prisma.user.findUnique({ where: { id }, include: { citizenprofile: true } });
    ...
    if (Object.keys(update_data).length > 0) { update_data.updated_at = new Date(); await prisma.user.update({ where: { id }, data: update_data }); }
    let profile_data = {};
    ...
    if (Object.keys(profile_data).length > 0) {
      if (user.citizenprofile) { await prisma.citizenProfile.update({ where: { id: user.citizenprofile.id }, data: profile_data }); } else { if (!profile_data.aadhaar_number) profile_data.aadhaar_number = `PENDING-${uuidv4().substring(0, 8)}`; await prisma.citizenProfile.create({ data: { ...profile_data, user_id: id } }); }
    }
...
exports.deleteCitizen = async (req, res) => {
  ...
  try {
    const id = req.params.id;
    const user = await prisma.user.findUnique({ where: { id }, include: { citizenprofile: true } });
    if (user && user.citizenprofile) { await prisma.citizenProfile.delete({ where: { id: user.citizenprofile.id } }); }
